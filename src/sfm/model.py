...
class SfMModel(nn.Module):
    ...
    def forward(self, images, neighbor_range=1):
        ...
        for i, image_i in enumerate(images):
            ...
            depth = self.depth_net.segmentation_head(self.depth_net.decoder(*depth_features[-1]))
            ...
            for j, _ in enumerate(images):
                if i != j and abs(i - j) <= neighbor_range:
                    pose_out = self.pose_decoder(
                        torch.cat([
                            self.pose_reduction(depth_features[i][-1]), 
                            self.pose_reduction(depth_features[j][-1])
                        ], dim=1))
