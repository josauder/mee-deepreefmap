...
parser.add_argument('--ignore_classes', type=str, default="background,fish,human", help='Classes to ignore when adding points to cloud')
parser.add_argument('--intrinsics_file', type=str, default="example_inputs/intrinsics.json", help='Path to intrinsics file')
parser.add_argument('--class_to_label_file', type=str, default="assigned_class_labels.json", help='Path to label_to_class_file')
parser.add_argument('--class_to_color_file', type=str, default="assigned_class_colors.json", help='Path to class_to_color_file')
...
def main(args):
    ...
    print("Ran NN Predictions in ", time() - t, "seconds, saving...")

    os.makedirs(args.out_dir + "/"+str(args.save_path)+"_videos", exist_ok=True)
    ...
    point_cloud.to_csv(args.out_dir + "/"+args.save_path + "_point_cloud.csv", index=False)
    ...
    tsdf_pc.to_csv(args.out_dir + "/"+args.save_path + "_point_cloud_tsdf.csv", index=False)
    print("Saved TSDF Point Cloud in ", time() - t, "seconds")


    print("Starting Benthic Cover Analsysis after ", time() - t, "seconds")
    results = benthic_cover_analysis(tsdf_pc, bins=args.output_2d_grid_size)
    np.save(args.out_dir + "/" + str(args.save_path) + ".npy", results)
    # TODO: export actual values from aggregated results
    print("Finished Benthic Cover Analysis in ", time() - t, "seconds")

    os.system("cp "+args.tmp_dir+"/*_.mp4 "+ args.out_dir + "/"+str(args.save_path)+"_videos")
    render_video(img_list, depths, semantic_segmentation, results, args.fps, class_to_color, class_to_label, args.tmp_dir)
    os.system("mv " + args.tmp_dir + "/out.mp4 " + args.out_dir + "/"+str(args.save_path)+"_videos")
    print("Rendered Video in ", time() - t, "seconds")



def get_nn_predictions(img_list, buffer_size, grav, num_classes, args):
    ...
    intrinsics = torch.tensor(json.load(open(args.intrinsics_file)).values()).float().to(device).unsqueeze(0)

    # Start by initializing, load the images in a buffer of buffer_size subsequent frames
    end_index = buffer_size
    images = [normalize(totensor(Image.open(img_list[i]))).to(device) for i in range(buffer_size)]

    semantic_segmentation = [segmentation_model(images[i]) for i in range(buffer_size)]
    ...
    while True:
        ...
        new_im = normalize(totensor(Image.open(img_list[end_index]))).to(device)
        with torch.no_grad():
            semantic_segmentation.append(segmentation_model(new_im))

        images.append(new_im.unsqueeze(0))

        with torch.no_grad():
            depth_features.append([f.detach() for f in sfm_model.extract_features(images[-1])])
        
        end_index += 1
        if end_index == len(img_list):
                break
    ...
    for i in range(buffer_size):
        ...
        if type(depths[end_index - i - 2]) == list:
            depth_uncertainties[end_index - i - 2] = np.std(depths[end_index - i - 2], axis=0)[0][0]
            depths[end_index - i - 2] = np.median(depths[end_index - i - 2], axis=0)[0][0]
            intrinsics_predicted[end_index - i - 2] = np.median(intrinsics_predicted[end_index - i - 2], axis=0)[0]
    ...
    return np.array(list(depths.values())), np.array(list([depth_uncertainties[i] for i in depths.keys()])), poses, semantic_segmentation, intrinsics_predicted
...
def get_point_cloud(image_list, depths, poses, depth_uncertainties, semantic_segmentation, intrinsics, label_to_color, class_to_label, args):
    ...
    def class_to_color(class_arr):
        ...
        for val in np.unique(class_arr):
            color_arr[class_arr[:,0]==val] = label_to_color[val]
        return color_arr
    ...
    with torch.no_grad(): 
        ...
        for i in tqdm(range(len(poses))):
            ...
            keep_mask = depth_i_tensor < args.distance_thresh
            ...
            for class_name in args.ignore_classes:
        ...
        xyz_index_arr = map_3d(np.concatenate([xyz_arr, distance2cam_arr, np.arange(len(xyz_arr))], axis=1), get_closest_to_centroid_with_attributes_of_closest_to_cam, 0.008)
        ...
        cloud = pd.DataFrame({
            ...
            "g": rgb_arr[:,4],
            "b": rgb_arr[:,5],
            "distance_to_cam": distance2cam_arr[:,6],
            "class": seg_arr[:,7],
            "class_r": class_color_arr[:,8],
            "class_g": class_color_arr[:,9],
            "class_b": class_color_arr[:,10],
            "frame_index": frame_index_arr[:,11],
            "depth_uncertainty": depth_unc_arr[:,12],
        })
...
def tsdf_point_cloud(img_list, depths, masks, poses, intrinsics, cutoff): 
    ...
    for i in tqdm(range(len(poses))):
        ...
        projected_img, projected_mask, projected_depth = rectify(
            ...
            torch.tensor(masks[i]*mask_out_background).unsqueeze(0).unsqueeze(0),
...
def benthic_cover_analysis(pc, bins=1000):
    #step 1: fit PCA
    ...
    img = np.zeros((int(np.max(out[:,0])+1), int(np.max(out[:,1])+1), 12))
    
    for gridcell in out:
        img[int(gridcell[0]), int(gridcell[1])] = gridcell[2:]
    return img
